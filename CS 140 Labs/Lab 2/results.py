...
class Process:
    ...
    def from_str(cls, s: str) -> Process:
        match = re.search(r"\[(?P<pid>\d+)\](\*|\s)?(?P<name>[^:]+):(?P<state>\d+)", s)
...
class TickLog:
    ...
    def from_lines(cls, lines: Sequence[str]) -> TickLog:
        return TickLog(
            tick_states=sorted(
                [TickState.from_line(line.strip()) for line in lines],
                key=lambda s: s.tick,
            ),
        )
    ...
    def get_ticks_with_pid(self, pid: int) -> Sequence[TickState]:
        return [
            tick_state
            for tick_state in self.tick_states
            if tick_state.is_pid_present(pid)
        ]
...
class MetricComputer:
    ...
    def get_response_time(self, tick_log: TickLog, pid: int) -> int:
        return self.get_first_execution_tick(tick_log, pid) - self.get_arrival_tick(
            tick_log, pid
        )

    def get_turnaround_time(self, tick_log: TickLog, pid: int) -> int:
        return self.get_ending_tick(tick_log, pid) - self.get_arrival_tick(
            tick_log, pid
        )
    ...
    def get_cpu_first_tick(self, tick_log: TickLog) -> int:
        ...
        raise RuntimeError("Log has no first tick")
    ...
    def get_cpu_last_tick(self, tick_log: TickLog) -> int:
        ...
        raise RuntimeError("Log has no last tick")
    ...
    def get_context_switches(self, tick_log: TickLog, pid: int) -> int:
        return sum(
            [
                1
                for tick_states in tick_log.get_ticks_with_pid(pid)
                if tick_states.processes.get_pid_state(pid) == State.RUNNING
            ]
        )

    def get_process_metrics(
        self, tick_log: TickLog, pid: int, policy: str, run: int
    ) -> ProcessMetrics:
    ...
    for pid in PIDS:
        for policy in POLICIES:
            for metric in METRICS:
                filtered = df[  # pyright: ignore
                    (df["pid"] == pid) & (df["policy"] == policy)
                ]
                ...
                stats.append(
                    MetricStats(
                        ...
                        policy=policy,
                        metric=metric,
                        mean=mean,  # pyright: ignore
                        median=median,  # pyright: ignore
                        range=data_range,
                        sd=sd,  # pyright: ignore
                    )
                )
...
def main():
    ...
    for policy in POLICIES:
        for n in range(1, N + 1):
            with open(f"output/{policy}-{n}.txt", "r") as f:
    ...
    for metric in METRICS:
        for policy in POLICIES_VS_BASELINE:
            for pid in PIDS:
                ...
                print(
                    f"{metric}, PID {pid}, baseline vs. {policy}:\t\t{scipy.stats.mannwhitneyu(x, y, alternative='two-sided')}"
                )
