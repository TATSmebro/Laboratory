  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        // print out relevant info regarding PCB.
        if (lab2_found > 0) {
          printf("%d", ticks);
          
          for (int k = 0; k <= highest_idx; k++) {
            p2 = &proc[k];
            if (p2->state == UNUSED) printf(" | [%d] ---:0", k);
            else if (p2->state == RUNNING) printf(" | [%d]*%s:%d", k, p2->name, p2->state);
            else printf(" | [%d] %s:%d", k, p2->name, p2->state);
          }
